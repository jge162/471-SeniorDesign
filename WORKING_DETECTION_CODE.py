...
def main():
    ...
    while True:
        ...
        for c in classes:
            ...
            confidence = c.score
            if class_label == 'SodaCan' and confidence > 0.10:
                # Check if the camera is already paused
                print("Trigger Arduino for Soda Can")
                ...
                break
            elif class_label == 'Water Bottle' and confidence > 0.10:
                # Check if the camera is already paused
                print("Trigger Arduino for Water Bottle")
                if not camera_paused:
                    # Pause the camera by setting the variable to True
                    camera_paused = True
                    # Trigger the recycling process
                ...
                # Exit the loop to prevent multiple instances of triggering
                break
            elif class_label == 'Paper Plate' and confidence > 0.10:
                # Check if the camera is already paused
                print("Trigger Arduino for Water Bottle")
                if not camera_paused:
                    # Pause the camera by setting the variable to True
